...
COPY --from=build /work/target/release/apme-web-leptos /usr/local/bin/apme-web-leptos
USER app
...
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/apme-web-leptos"]
